...
def document_tool(
    chat_id: Annotated[str, "A string containing the Chat ID"],
    query: Annotated[str, "A string query to retrieve relevant information from the document/file"]
) -> str:
    """
    Retrieves relevant information from an attached document/file based on the provided query.

    Returns:
        str: A string containing the relevant information retrieved from the document, 
             or a message stating that no document is attached.
    """
    try:
        chat = Chat.objects.get(id=chat_id)
    except Chat.DoesNotExist:
        return "Chat not found! Check the Chat ID"    

    if chat.attach is not None:
        return AIVectorDB().get_context(chat.attach.vector_db_path, query)
    return "No document attached"

tool_status_messages = {
    "duckduckgo_search_tool": "Searching DuckDuckGo...",
    "wikipedia_tool": "Searching Wikipedia...",
    "web_url_tool": "Scraping the web page...",
    "madhu_biography_tool": "Retrieving Madhu's biography...",
    "calculator_tool": "Calculating the expression...",
    "document_tool" : "Retrieving information from the document..."
}
